...
filter(titanic, Sex == "male" & str_detect(Name, "George|Frank") & Survived)
...
arrange(titanic, desc(Age)) %>% filter(Sex == "female" & Pclass == 1) %>% top_n(5, Age)
```
...
arrange(titanic, desc(Age)) %>% slice(1:10) %>% select(Name:Fare)
# za nasumično odabranih 1% redaka ispišite sve atribute osim identifikatora 
# putnika i broja kabine
sample_frac(titanic, 0.01) %>% select(-c(PassengerId, Cabin))
...
titanic <- mutate(titanic, hadRelativesOnBoard = as.logical(SibSp + Parch))
...
filter(titanic, Age > 20 & Sex == "female" & Embarked == "S") %>% sample_n(10) %>% transmute(Name, Pclass, FareToday = sprintf("$%f", round(Fare * 23.85, 2))) %>% arrange(desc(Pclass))
...
titanic %>% sample_n(20) %>% select(Name, Pclass, Fare, FareCategory) %>% arrange(FareCategory)
...
summarise(flights815, mean(ArrDelay))
...
filter(hflights, CancellationCode == "B") %>% group_by(Month) %>% summarise("BadWeatherCancellations" = sum(Cancelled))
...
filter(hflights, Dest == "LAX" | Dest == "JFK" | Dest == "LGA") %>% group_by(Dest, DayOfWeek) %>% summarise("MeanArrDelay" = mean(ArrDelay, na.rm = T)) %>% arrange(desc(MeanArrDelay))
...
group_by(ArrDelayCatId) %>%
summarise(TotalFlights = n(), minArrDelay = min(ArrDelay), maxArrDelay = max(ArrDelay), mostFreqDest = mostFreqValue(Dest))

# ponovite prethodni primjer ali umjesto funkcije 
# `ntile` isprobajte funkciju `cut`
mutate(hflights, ArrDelayCatId = cut(ArrDelay,10)) %>%
group_by(ArrDelayCatId) %>%
summarise(TotalFlights = n(), minArrDelay = min(ArrDelay), maxArrDelay = max(ArrDelay), mostFreqDest = mostFreqValue(Dest))
```
